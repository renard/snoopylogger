...
static inline void snoopy_log(const char *filename, char *const argv[])
{
	...
	openlog("snoopy", LOG_PID, LOG_AUTHPRIV);
		...
		syslog(LOG_INFO, "[uid:%d sid:%d cwd:%s path:%s]: %s", filename, getuid(), getsid(0), cwd, logString);
	#else
		syslog(LOG_INFO, "[uid:%d sid:%d path:%s]: %s", filename, getuid(), getsid(0), logString);
	#endif
